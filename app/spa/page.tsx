...
function useStats() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ usersOnline: number; sessionsToday: number; cpuLoad: number }>;
    },
  });
}
...
function useBookmarks() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<Array<{ id: string; title: string; tags: string[] }>>;
    },
  });
}
      ...
      <div className="p-4 max-w-3xl mx-auto">
        <header className="mb-6 flex items-center justify-between">
          ...
          <nav className="flex gap-3 text-sm">
            ...
            <Link to="/bookmarks" className="underline">Bookmarks</Link>
          </nav>
        </header>

        <div className="mb-4 text-xs text-muted-foreground">
          Hash routing ensures this SPA lives under Next.js route "/spa" without conflicts.
        </div>
        ...
        <div className="mb-4 flex gap-2 text-xs">
          <button
            ...
            onMouseEnter={() => {
              ...
              qc.prefetchQuery({ queryKey: ["stats", "summary"], queryFn: () => fetch("/api/mock/stats/summary").then(r => r.json()) });
            }}
          >
            Prefetch stats
          </button>
...
const Index = createRoute({
  ...
  component: function IndexPage() {
    return (
      <div>
        <p className="mb-2">Welcome to the hash-based SPA. Navigate to Dashboard or Bookmarks.</p>
      </div>
    );
  },
});
  ...
  component: function DashboardPage() {
    ...
    return (
      <div>
        ...
        {error && <p className="text-red-500">{String((error as Error).message)}</p>}
        {data && (
      ...
      <div>
        ...
        {data && (
          <ul className="list-disc pl-5 text-sm">
            {data.map((b) => (
              <li key={b.id}>
                {b.title} {b.tags?.length ? <span className="text-muted-foreground">({b.tags.join(", ")})</span> : null}
              </li>
            ))}
          </ul>
        )}
