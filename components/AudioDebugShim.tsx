...
export default function AudioDebugShim() {
  useEffect(() => {
    try {
      ...
      if (OriginalAC)
        (window as any).AudioContext = wrapCtor(OriginalAC, "AudioContext");
      if (OriginalWkAC)
      ...
      if (OriginalCreate && !OriginalCreate.__WRAPPED) {
        (window as any).AudioContext.prototype.createMediaStreamSource =
          function (this: BaseAudioContext, stream: MediaStream) {
            try {
        ...
        (
          window as any
        ).AudioContext.prototype.createMediaStreamSource.__WRAPPED = true;
      }
    } catch (e) {
