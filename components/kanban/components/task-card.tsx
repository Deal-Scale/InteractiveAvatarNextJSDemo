...
export function TaskCard({ task, isOverlay }: TaskCardProps) {
	...
	}, [confettiKey, task.status]);

	return (
		<Card
			ref={setNodeRef}
			style={style}
			className={`${variants({ dragging: isOverlay ? "overlay" : isDragging ? "over" : undefined })} relative min-w-0 max-w-full overflow-hidden`}
		>
			<DragHeader
			...
			<CardContent className="min-w-0 max-w-full whitespace-pre-wrap px-3 pt-3 pb-6 text-left">
				<div className="min-w-0 break-words font-semibold text-lg">
					{task.title}
				</div>
				{task.description && (
					<div className="mt-2 min-w-0 break-words text-muted-foreground text-sm">
						{task.description}
					</div>
				)}
								...
								<CollapsibleTrigger asChild>
									<button type="button" className="w-full text-left">
										<div className="flex items-center justify-between rounded-md border bg-amber-50 px-3 py-2 text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
											<div className="font-semibold text-sm">From AI</div>
									...
									<div className="mt-3 grid gap-3">
										{task.outputVideoUrl && (
											<div className="text-sm">
												...
												<video
													controls
													className="mt-1 max-h-64 w-full rounded border"
												>
										...
										{task.outputMarkdown && (
											<div>
												...
												<div
													...
													// biome-ignore lint/security/noDangerouslySetInnerHtml: Markdown is escaped before limited formatting is applied.
