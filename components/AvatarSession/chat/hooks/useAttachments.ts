...
export function useAttachments(opts: {
  ...
  const { composerAttachments, clearComposerAttachments, onSendMessage, onChatInputChange } = opts;
  ...
  const sendWithAttachments = (text: string) => {
    ...
    if (!trimmed && attachments.length === 0 && composerAttachments.length === 0) return;
    ...
    console.debug("[Chat] built outgoing payload", { text: `${trimmed}${suffix}`, assets });
  ...
  return { attachments, onFilesAdded, removeAttachment, sendWithAttachments } as const;
}
