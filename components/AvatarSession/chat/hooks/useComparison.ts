...
export function useComparison(messages: Message[]) {
	...
	const [compareAlternative, setCompareAlternative] = useState<
		string | undefined
	>(undefined);
	...
	const handleCompare = (content: string, id: string) => {
		...
		const currentAvatarCount = messages.filter(
	...
	useEffect(() => {
		...
		const avatarMsgs = messages.filter(
			(m) => m.sender === MessageSender.AVATAR,
		);
	...
	const handleChooseComparison = (choice: "A" | "B") => {
		const chosen =
			choice === "A" ? compareOriginal : (compareAlternative ?? "");
		if (!chosen) return;
