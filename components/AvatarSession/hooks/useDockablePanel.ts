...
export function useDockablePanel(
	...
	const [hydrated, setHydrated] = useState<boolean>(() => {
		...
		try {
			const api = (usePlacementStore as unknown as { persist?: PersistAPI })
				.persist;
		...
		const onMove = (e: PointerEvent) => {
			...
			if (resizing === "bottom") {
				...
				if (Math.abs((bottomHeightFrac || 0) - nextFrac) > 0.001) {
					console.debug("[dockable] resize bottom -> setBottomHeightFrac", {
						nextFrac,
					});
				...
				if (Math.abs(rightWidthFrac - nextFrac) > 0.001) {
					console.debug("[dockable] resize right -> setRightWidthFrac", {
						nextFrac,
					});
				...
				if (Math.abs(sizeW - nextW) > 0.5 || Math.abs(sizeH - nextH) > 0.5) {
					console.debug("[dockable] floating resize -> setWindowSize", {
						nextW,
						nextH,
					});
	...
	useEffect(() => {
		...
		if (dock === "right" && rightPct <= 0) {
			...
			if (Math.abs((rightWidthFrac || 0) - next) > 0.001) {
				console.debug("[dockable] ensure visible -> right setRightWidthFrac", {
					next,
				});
	...
	return {
		...
		setDock: (mode: DockMode) => {
			...
			if (next !== mode) {
				console.debug("[dockable] mobile restriction: coerce setDock", {
					requested: mode,
					next,
				});
			}
		...
		setBottomSize: (pct: number) =>
			setBottomHeightFrac(
				Math.max(MIN_BOTTOM_SIZE_PCT, Math.min(100, pct)) / 100,
			),
