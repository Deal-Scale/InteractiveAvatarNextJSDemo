...
export function useAvatarOptions(selectedId?: string) {
	const [avatarOptionsState, setAvatarOptions] =
		useState<AvatarOption[]>(AVATARS);
	...
	const { data: avatarsFromApi } = useQuery({
		...
		queryFn: async () => {
			...
			const list: AvatarApiItem[] = Array.isArray(json?.data) ? json.data : [];
			return list.map((a: AvatarApiItem) => ({
	...
	const avatarOptions = avatarsFromApi?.length
		? avatarsFromApi
		: avatarOptionsState;
