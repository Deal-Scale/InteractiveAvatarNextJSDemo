...
export const AvatarConfig: React.FC<AvatarConfigProps> = ({
	...
	const onChange = <T extends keyof StartAvatarRequest>(
		...
		try {
			...
			if (key === "language") {
				...
				const newVoice: AgentConfig["voice"] = {
					...(currentAgent?.voice ?? {}),
					voiceId: v?.voiceId,
					rate: v?.rate,
					emotion: v?.emotion as unknown as NonNullable<
						AgentConfig["voice"]
					>["emotion"],
					elevenlabs_settings: {
						...(currentAgent?.voice?.elevenlabs_settings ?? {}),
						model_id:
							(v?.model as unknown as string) ??
							(currentAgent?.voice?.elevenlabs_settings?.model_id as
								| string
								| undefined),
					},
				} as AgentConfig["voice"];
				patch.voice = newVoice;
				// Also mirror the top-level voiceId if present in schema
				if (v?.voiceId) {
					(patch as unknown as { voiceId?: string }).voiceId = v.voiceId;
				}
			}
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: We intentionally only react to currentAgent changes to avoid noisy config churn.
	useEffect(() => {
		...
		const merged: StartAvatarRequest = {
			...
			voice: {
				...
				emotion:
					(currentAgent.voice?.emotion as unknown as NonNullable<
						StartAvatarRequest["voice"]
					>["emotion"]) ?? config.voice?.emotion,
				model:
					(currentAgent.voice?.elevenlabs_settings?.model_id as unknown as
						| string
						| undefined) ??
					(config.voice?.model as unknown as string | undefined),
			},
