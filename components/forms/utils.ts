...
export type Widget = "input" | "number" | "textarea" | "select" | "switch" | "slider";
...
export function unwrapType(t: z.ZodTypeAny): z.ZodTypeAny {
  ...
  while (
    ...
    if (cur?._def?.innerType) cur = cur._def.innerType;
    else if (cur?._def?.schema) cur = cur._def.schema;
    else break;
  }
...
export function booleanSelectOptions(custom?: Array<{ value: string; label: string }>) {
  return (
    custom ?? [
...
export function isSensitiveString(def: any, cfg?: FieldConfig) {
  ...
  return (
    desc.includes("sensitive") || desc.includes("password") || cfg?.widget === "password"
  );
}
