...
export async function postMcpPrompt(name: string, args: Record<string, unknown>) {
...
export function useMcpToolMutation() {
  return useStandardMutation((vars: { name: string; args: Record<string, unknown> }) =>
    postMcpTool(vars.name, vars.args),
  );
}
export function useMcpResourceMutation() {
  return useStandardMutation((vars: { uri: string }) => postMcpResource(vars.uri));
}
