...
import { useAssetsStore, type AssetItem, type UploadProgress } from "@/lib/stores/assets";
...
function validateFiles(input: File[] | FileList) {
  ...
  if (files.length > MAX_FILES) throw new Error(`Too many files. Max ${MAX_FILES} per upload.`);
  for (const f of files) {
    if (typeof f.size === "number" && f.size > MAX_SIZE_BYTES) {
      throw new Error(`File '${f.name}' exceeds 10MB limit.`);
    }
    if (!ALLOWED_MIME.has(f.type)) {
      throw new Error(`File '${f.name}' has unsupported type '${f.type || "unknown"}'.`);
    }
  }
  return files;
}
...
export function useAssetsUploadMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (assets, _vars, ctx) => {
      ...
      if (ctx?.temps?.length) {
        setUploads((s) => ({ uploads: s.uploads.filter((u) => !ctx.temps.find((t) => t.id === u.id)) }));
      }
